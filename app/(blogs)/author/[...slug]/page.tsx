...
import { formatDate, formatSlug } from "@/lib/utils"
import { SidebarNavItem } from "@/types"
import { allAuthors, allCategories, allPosts, Author, Category, Post } from "contentlayer/generated"
...
async function getAuthorFromParams(params: Params): Promise<Author | undefined> {
    const { slug } = await params
    const slugs = slug.join("/")
    return allAuthors.find((author) => author.slugAsParams == slugs)
}
...
async function getMacthedPostFromParams(params: Params) {
    ...
    const posts: Post[] = allPosts.filter((post) =>
        post.authors.find((postauthor) => formatSlug(postauthor) == slugs))
...
export default async function PostPage(props: {
    ...
    const curAuthor = await getAuthorFromParams(props.params)
    if (!curAuthor) {
        notFound()
    }
